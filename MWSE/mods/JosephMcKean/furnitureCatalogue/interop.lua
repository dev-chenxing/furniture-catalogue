...
--[[
	...
	if furnitureCatalogue then
		furnitureCatalogue.registerFurniture({
			["551"] = { id = "ab_furn_shalk_01", name = "Shalk", category = "Other", cost = 60, ashlandersOnly = true },
			["552"] = { id = "flora_ashtree_05", name = "Tree, Ashland", category = "Plants", cost = 120, ashlandersAvailable = true },
		})
	end
	...
	key ("551" in the example): the index of the new furniture. 
	...
	id: the object id of the new furniture.
	name: the name of the new furniture.
	category: the category of the new furniture. I suggest you use the existing ones.
	cost: the price of the new furniture.
	ashlandersAvailable: if the furniture can be purchased from Ashlander furniture merchants
	ashlandersOnly: if the furniture can be only purchased from Ashlander furniture merchants. Implies ashlandersAvailable is true.
...
local function registerFurniture(e)
	...
	for index, furnitureData in pairs(e.data) do
		...
		assert(type(furnitureData.cost) == 'number', "cost must be a name")
